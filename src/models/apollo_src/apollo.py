"""Apollo model definition — vendored verbatim (inference only).

Source: JusperLee/Apollo (https://github.com/JusperLee/Apollo) look2hear/models/apollo.py
License: CC-BY-SA 4.0. Only edit vs upstream: removed a debug print() in __init__.
"""
...
class Apollo(BaseModel):
    ...
    def spec_band_split(self, input):
        ...
        # VentiPlayer fp16 patch: n_fft=882 is not a power of two, and cuFFT
        # rejects non-pow2 sizes in half precision. Force fp32 for the STFT.
        with torch.autocast(device_type=input.device.type, enabled=False):
            spec = torch.stft(input.view(B*nch, nsample).float(), n_fft=self.win,
    ...
    def forward(self, input):
        ...
        for i in range(self.nband):
            ...
            # VentiPlayer fp16 patch: build the complex spectrogram in fp32 so the
            # iSTFT (non-pow2 n_fft) doesn't hit cuFFT's ComplexHalf restriction.
            this_RI = this_RI.float()
