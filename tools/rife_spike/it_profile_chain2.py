"""Phase 1.5 性能归因 v2：与生产模板同构（worker 推理线程 + 求值期预热），
分阶段计时 + --untimed 墙钟吞吐。

用法: python it_profile_chain2.py [--video xxx] [--model v4_25_lite] [--scale 0.5]
"""
import os
import sys
import time
...
_box = _submit(_warm, _warm)
_box["ev"].wait()
del _warm, _box

STATS = {{"n": 0, "read": 0.0, "h2d": 0.0, "down": 0.0, "wait": 0.0,
    ...
    if s["n"] % 120 == 0:
        ...
        with open(PROF, "a", encoding="utf-8") as fh:
            fh.write(f"n={{s['n']}} wall={{el:.1f}}s pairs_per_s={{s['n']/el:.1f}} "
                     ...
                     f"down={{s['down']/s['n']*1e3:.1f}} wait={{s['wait']/s['n']*1e3:.1f}} "
